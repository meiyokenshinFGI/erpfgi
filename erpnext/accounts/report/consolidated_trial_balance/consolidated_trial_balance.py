...
from erpnext.accounts.report.consolidated_financial_statements import filter_accounts, get_gl_entries
...
def get_data(filters):
	accounts = frappe.db.sql("""select * from `tabAccount` where consolidation=%s order by lft""",
		filters.consolidation, as_dict=True)
	...
	min_lft, max_rgt = frappe.db.sql("""select min(lft), max(rgt) from `tabAccount`
		where consolidation=%s""", (filters.consolidation,))[0]

	gl_entries_by_account = get_gl_entries(filters.consolidation, None, filters.to_date, min_lft, max_rgt,
