...
def get_data(consolidation, root_type, balance_must_be, period_list, ignore_closing_entries=False):
	accounts = get_accounts(consolidation, root_type)
	if not accounts:
		return None

	accounts, accounts_by_name = filter_accounts(accounts)
	gl_entries_by_account = get_gl_entries(consolidation, period_list[0]["from_date"], period_list[-1]["to_date"],
...
def get_gl_entries(consolidation, from_date, to_date, root_lft, root_rgt, ignore_closing_entries=False):
	...
	gl_entries = frappe.db.sql("""select * from `tabGL Entry`
		where consolidation=%(consolidation)s
		{additional_conditions}
